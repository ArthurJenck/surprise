...
import { createShakePattern, sampleShakePattern, type ShakePattern } from './shakePattern'
...
async function loadGiftAsset(config: SceneConfig, themeConfig: ThemeConfig): Promise<GiftAsset> {
  ...
  gltf.scene.traverse((object) => {
    ...
    object.material = Array.isArray(object.material)
      ? objectMaterials.map((material) => recolorGiftMaterial(material, config, themeConfig))
...
function recolorGiftMaterial(
  ...
  if (!(recoloredMaterial instanceof THREE.MeshStandardMaterial) || !recoloredMaterial.map) {
...
function createShadow(config: SceneConfig, themeConfig: ThemeConfig) {
  ...
  const shadow = new THREE.Mesh(
    new THREE.PlaneGeometry(config.gift.shadow.size[0], config.gift.shadow.size[1]),
    material
  )
...
function createShadowTexture(config: SceneConfig, themeConfig: ThemeConfig) {
  ...
  gradient.addColorStop(config.gift.shadow.middleStop, themeConfig.scene.shadowMiddle)
